...
```
## Notice: you can generate your own password string with ```openssl passwd -1 mysecretpassword``` but note that ```$``` needs to be escaped with ```\```as shown in the example.
```bash
